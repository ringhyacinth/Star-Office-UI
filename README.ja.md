...
一部の API は Token 認証が必要です：

| Header | 用途 |
|--------|------|
| `X-API-Token` | ユーザー API Token（状態読み書き） |
| `X-Admin-Token` | 管理者 Token（管理操作） |
...
Star Office UI は OpenClaw プラグインを提供し、Agent 状態の自動同期をサポートします。2つの使用モードがあります：

### 5.1 2つの使用モード

| モード | 説明 | ユースケース |
|--------|------|-------------|
| **ローカル参加** | Join Key でオフィスに参加 | マルチエージェント協力、ローカルデプロイ |
| **リモート接続** | プラグイン設定でリモートオフィスに接続 | パブリックアクセス、リモート監視 |
...
# リモートプラグインをインストール（特定バージョン）
openclaw plugins install star-office-ui@1.0.0
```

> ⚠️ **セキュリティ注意**：プラグインインストールはコード実行として扱います。固定バージョンの使用を推奨。
...
|-----------|--------|------|-----------|------|
| `apiUrl` | string | ✅ 必須 | `http://localhost:5000` | Star Office UI バックエンド API URL |
| `apiToken` | string | ✅ 必須 | - | API 認証 Token（バックエンドで生成） |
| `agentId` | string | 任意 | `openclaw-{timestamp}` | Agent 一意識別子 |
| `agentName` | string | 任意 | - | Agent 表示名 |
| `joinKey` | string | 任意 | - | リモートモードでは空可 |
| `memoryDir` | string | 任意 | `~/.openclaw/workspace/memory` | memory ファイルディレクトリ |
| `autoIdleSeconds` | number | 任意 | `300` | 自動アイドルタイムアウト（秒） |
...
# 2. Token 生成（先に ADMIN_TOKEN を設定が必要）
curl -X POST http://localhost:5000/api/v1/admin/token/generate \
  -H "X-Admin-Token: your-admin-token" \
...
- ✅ `agent_error` - エラー発生時にエラー状態を表示
- ✅ 昨日のメモをオフィスダッシュボードに自動同期
- ✅ 毎日8時に自動同期（cron または heartbeat 経由）
