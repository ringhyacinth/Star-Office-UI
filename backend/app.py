...
if os.path.exists("/app/memory"):
    ...
    TOKENS_FILE = "/app/api-tokens.json"
    ADMIN_TOKEN_FILE = "/app/admin-token.json"
else:
    ...
    ADMIN_TOKEN_FILE = os.path.join(ROOT_DIR, "admin-token.json")
...
# Initialize Plugin Manager after Flask app is created
...
def load_admin_token():
    ...
    if os.path.exists(ADMIN_TOKEN_FILE):
        try:
            ...
            pass
    # 生成默认 admin token（仅首次）
    default_admin_token = secrets.token_urlsafe(32)
    save_admin_token(default_admin_token)
    return {"admin_token": default_admin_token}


def save_admin_token(token):
    """保存 Admin Token"""
    with open(ADMIN_TOKEN_FILE, "w", encoding="utf-8") as f:
        json.dump({"admin_token": token}, f, ensure_ascii=False, indent=2)


def require_api_token(f):
    """API Token 验证中间件（用于普通 API 访问）"""
    @wraps(f)
    def decorated(*args, **kwargs):
        ...
        valid_tokens = [t.get("token") for t in tokens_data.get("tokens", []) if t.get("enabled", True)]
...
def require_admin_token(f):
    """Admin Token 验证中间件（用于管理操作）"""
    @wraps(f)
    def decorated(*args, **kwargs):
        token = request.headers.get("X-Admin-Token")
        if not token:
            return jsonify({"error": "Missing X-Admin-Token header"}), 401
        
        admin_data = load_admin_token()
        if token != admin_data.get("admin_token"):
...
def get_today_memo():
    ...
    try:
        ...
        if os.path.exists(today_file):
            memo_content = extract_memo_from_file(today_file)
            return jsonify({"success": True, "date": today_str, "memo": memo_content})
        else:
            return jsonify({"success": False, "msg": "还没有写今日日记"})
    except Exception as e:
        return jsonify({"success": False, "msg": str(e)}), 500
...
def generate_token():
    ...
    try:
        ...
        name = data.get("name", "").strip() or f"Token-{datetime.now().strftime('%Y%m%d%H%M%S')}"
    ...
    try:
        ...
        for t in tokens_data.get("tokens", []):
            tokens.append({
                ...
                "token_preview": t.get("token", "")[:8] + "..." if t.get("token") else ""
            })
...
@app.route("/api/v1/admin/token/<path:token>", methods=["DELETE"])
@require_admin_token
def revoke_token(token):
    """撤销指定的 API Token"""
    try:
        tokens_data = load_tokens()
        original_count = len(tokens_data["tokens"])
        tokens_data["tokens"] = [t for t in tokens_data["tokens"] if t.get("token") != token]
        
        if len(tokens_data["tokens"]) == original_count:
...
@require_admin_token
def get_admin_token_info():
    """获取当前 Admin Token 信息"""
    try:
        admin_data = load_admin_token()
        return jsonify({
            "success": True,
            "has_admin_token": bool(admin_data.get("admin_token"))
        })
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
